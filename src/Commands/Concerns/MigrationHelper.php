...
namespace Zahzah\LaravelSupport\Commands\Concerns;
...
trait MigrationHelper{
    ...
    protected function findAppropriateStub(){
        if (isset($this->__prefix)){
            switch ($this->__prefix) {
                ...
                break;
                case 'create_'          : $this->__stub = $this->chooseCreateStub();break;
            }
        }else{
    ...
    protected function chooseCreateStub(): string{
        switch ($this->__suffix) {
            case '_table'  : $stub = "migration.create";break;
            default        : $stub = 'migration'.\str_replace('_','.',$this->__suffix);break;
        }
