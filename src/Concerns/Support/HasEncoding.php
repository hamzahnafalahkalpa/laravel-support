...
trait HasEncoding
{
    ...
    protected static function resetIncrementIfNewPeriod($resetable, &$part)
    {
        ...
        if (isset($date) && isset(static::$__encoding_schema->value)) {
            ...
            switch ($resetable) {
                case 'year':
                    $notReset = !($currentDate->year !== $currentFormattedDate->year);
                    break;
                case 'month':
                    $notReset = !($currentDate->year !== $currentFormattedDate->year || $currentDate->month !== $currentFormattedDate->month);
                    break;
                case 'day':
                    $notReset = !(!$currentDate->isSameDay($currentFormattedDate));
                    break;
            }
        }
    ...
    {
        return $this->morphOneModel('ModelHasEncoding', 'reference');
    }
    public function modelHasEncodings()
    {
        return $this->morphManyModel('ModelHasEncoding', 'reference');
    }
